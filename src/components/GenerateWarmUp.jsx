...
import { AI_PROVIDERS, getAIProviderManager } from "../utils/aiProviderFactory";
...
const GenerateWarmUp = ({ onCourseClick, onCurriculumGenerated }) => {
  ...
  const [selectedProvider, setSelectedProvider] = useState(() => {
    return localStorage.getItem('preferredAIProvider') || process.env.REACT_APP_DEFAULT_AI_PROVIDER || AI_PROVIDERS.GEMINI;
  });
  ...
  useEffect(() => {
    try {
      ...
      // If selected provider is not available, switch to first available
  ...
  const handleProviderChange = (e) => {
    ...
    setProviderFeedback(null); // Clear any previous feedback
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (generateWarmUp) {
        const warmUpResult = await run(userInput, coursesArray, 'warmUp', criteria, selectedProvider);
        if (warmUpResult !== undefined) {
          setWarmUpResponse(warmUpResult.content);

          // Show provider feedback if fallback was used
          if (warmUpResult.metadata?.usedFallback) {
            setProviderFeedback({
              type: 'warning',
              message: `Note: ${warmUpResult.metadata.fallbackReason === 'rate_limit' ? 'Rate limit reached on' : 'Error with'} ${selectedProvider.toUpperCase()}. Automatically switched to ${warmUpResult.metadata.providerName}.`
            });
          } else {
            setProviderFeedback({
              type: 'info',
              message: `Generated by ${warmUpResult.metadata.providerName}`
            });
          }
        }
      }
      ...
      if (generateLessonPlan) {
        const lessonPlanResult = await run(userInput, coursesArray, 'lessonPlan', criteria, selectedProvider);
        if (lessonPlanResult !== undefined) {
          setLessonPlanResponse(lessonPlanResult.content);

          // Show provider feedback if fallback was used (only if not already set from warm-up)
          if (!providerFeedback) {
            if (lessonPlanResult.metadata?.usedFallback) {
              setProviderFeedback({
                type: 'warning',
                message: `Note: ${lessonPlanResult.metadata.fallbackReason === 'rate_limit' ? 'Rate limit reached on' : 'Error with'} ${selectedProvider.toUpperCase()}. Automatically switched to ${lessonPlanResult.metadata.providerName}.`
              });
            } else {
              setProviderFeedback({
                type: 'info',
                message: `Generated by ${lessonPlanResult.metadata.providerName}`
              });
            }
          }
        }
  ...
  const handle1stButtonClick = async () => {
    ...
    try {
      if (generateWarmUp) {
        const warmUpResult = await run(exampleText, coursesArray, 'warmUp', criteria, selectedProvider);
        if (warmUpResult !== undefined) {
          setWarmUpResponse(warmUpResult.content);
          if (warmUpResult.metadata?.usedFallback) {
            setProviderFeedback({ type: 'warning', message: `Switched to ${warmUpResult.metadata.providerName} due to ${warmUpResult.metadata.fallbackReason}` });
          }
        }
      }
      if (generateLessonPlan) {
        const lessonPlanResult = await run(exampleText, coursesArray, 'lessonPlan', criteria, selectedProvider);
        if (lessonPlanResult !== undefined) setLessonPlanResponse(lessonPlanResult.content);
      }
    } catch (error) {
      console.error("Error generating AI response:", error);
    ...
    const handle2ndButtonClick = async () => {
      ...
      try {
        if (generateWarmUp) {
          const warmUpResult = await run(exampleText, coursesArray, 'warmUp', criteria, selectedProvider);
          if (warmUpResult !== undefined) setWarmUpResponse(warmUpResult.content);
        }
        if (generateLessonPlan) {
          const lessonPlanResult = await run(exampleText, coursesArray, 'lessonPlan', criteria, selectedProvider);
          if (lessonPlanResult !== undefined) setLessonPlanResponse(lessonPlanResult.content);
        ...
        <div className="ai-input-field">
          ...
          {availableProviders.length > 0 && (
            <div className="mb-4 p-3 bg-light border rounded">
              <h5 className="mb-3">AI Provider</h5>
              <FormGroup>
                <Label for="aiProvider" className="fw-bold">
                  Select AI Provider:
                </Label>
                <Input
                  type="select"
                ...
                >
                  {availableProviders.map(provider => (
                    <option key={provider} value={provider}>
                      {provider === AI_PROVIDERS.GEMINI ? 'Google Gemini' : 'OpenAI GPT'}
                    </option>
                  ))}
                </Input>
                <small className="text-muted d-block mt-2">
                  If rate limits are reached, the system will automatically fallback to the alternate provider.
                </small>
              </FormGroup>
            </div>
          )}

          {/* Provider Feedback Alert */}
          {providerFeedback && (
            <Alert color={providerFeedback.type} className="mb-4" dismissible onClose={() => setProviderFeedback(null)}>
